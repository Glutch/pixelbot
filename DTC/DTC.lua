...
DTC = {}
DTC = LibStub("AceAddon-3.0"):NewAddon("AceConsole-3.0", "AceEvent-3.0", "AceTimer-3.0", "AceComm-3.0", "AceSerializer-3.0")
...
function DTC:StringToASCIIHex(str)
...
function DTC:CreateFrames(n)
    ...
    local function UpdateFrameColor(f)
        ...
        if not SETUP_SEQUENCE then
            ...
            MakePixelSquareArr(fixedDecimalToColor(DTC:GetPlayerFacing()), 3) --3 The direction the player is facing in radians
            -- glutch MakePixelSquareArr(integerToColor(self:GetZoneName(0)), 4) -- Get name of first 3 characters of zone
            -- glutch MakePixelSquareArr(integerToColor(self:GetZoneName(3)), 5) -- Get name of last 3 characters of zone
            ...
            -- glutch     MakePixelSquareArr(integerToColor(self:itemName(i, itemNum)), 20 + i * 2)
                -- Return item slot number
            -- glutch     MakePixelSquareArr(integerToColor(i * 16 + itemNum), 21 + i * 2)
            -- glutch end
            ...
            MakePixelSquareArr(integerToColor(self:isTargetMob()), 52)
            ...
            -- glutch MakePixelSquareArr(integerToColor(self:GameTime()), 44) -- Returns time in the game
            -- glutch MakePixelSquareArr(integerToColor(self:GetGossipIcons()), 45) -- Returns which gossip icons are on display in dialogue box
            -- glutch MakePixelSquareArr(integerToColor(self:PlayerClass()), 46) -- Returns player class as an integer
            -- glutch MakePixelSquareArr(integerToColor(self:isUnskinnable()), 47) -- Returns 1 if creature is unskinnable
            -- glutch MakePixelSquareArr(integerToColor(self:hearthZoneID()), 48) -- Returns subzone of that is currently bound to hearhtstone
            self:HandleEvents()
        end
    ...
    local function setFramePixelBackdrop(f)
        ...
        f:SetBackdrop({
            bgFile = "Interface\\AddOns\\DTC\\white.tga",
...
function DTC:GetCurrentPlayerPosition()
...
function DTC:Base2Converter()
    ...
    self:MakeIndexBase2(self:GetBuffs("Power Word: Shield"), 12) + self:MakeIndexBase2(self:GetBuffs("Drink"), 13) + self:MakeIndexBase2(self:playerCombatStatus(), 14) +
...
function DTC:MakeIndexBase2(bool, power)
...
function DTC:GetTargetName(partition)
    -- Uses wow function to get target string
    local target = GetUnitName("target")
    if target ~= nil then
        target = DTC:StringToASCIIHex(target)
        if partition < 3 then
...
function DTC:getPetHp()
    local max = UnitHealthMax("pet")
    local curr = UnitHealth("pet")
    if (max == 0) then
        return 0
    end
...
function DTC:itemName(bag, slot)
    local item
    local itemCount
...
function DTC:returnItemFromIndex(index)
    return items[index]
end

function DTC:enchantedItems()
    if ValuesAreEqual(items, itemsPlaceholderComparison) then
    end
end

function DTC:equipName(slot)
    local equip
...
function DTC:IsTargetElemental()
...
function DTC:needAmmo()
    local numberOfFreeSlots, BagType = GetContainerNumFreeSlots(4);
    return numberOfFreeSlots
end

function DTC:IsTargetTooHighLevel()
    if (UnitLevel("target") == -1) then
        return 1
    end
    
    if (UnitLevel("target") > UnitLevel("player") + 2) then
        return 1
    else
        return 0
    end
...
function DTC:GetPetHappiness(debuff_name)
    if (GetPetHappiness() == nil) then
        return 1
    end
...
function DTC:GetPetIsDead()
    if (UnitIsDead("pet") == true) then
        return 1
    else
        return 0
    end
...
function DTC:GetPlayerDebuffs(debuff_name)
    for i=1,5 do
...
function DTC:GetPlayerBuffs(buff_name)
    for i=1,10 do
        local name, rank, iconTexture, count, debuffType, duration, timeLeft = UnitBuff("player", i); 
        if name == buff_name and duration ~=nil then
            return 1
        end
    end
    return 0
...
-- Returns zone name
function DTC:GetZoneName(partition)
    local zone = DTC:StringToASCIIHex(GetZoneText())
...
function DTC:GameTime()
    local hours, minutes = GetGameTime()
    hours = (hours * 100) + minutes
    return hours
end

function DTC:GetGossipIcons()
...
function DTC:CorpsePosition(coord)
...
function DTC:targetCombatStatus()
...
function DTC:checkTalentPoints()
...
function DTC:playerCombatStatus()
...
function DTC:GetInventoryBroken()
    for i = 1, 16 do
...
function DTC:IsTargetOfTargetPlayer()
...
function DTC:ProcessExitStatus()
...
function DTC:HandlePartyInvite()
