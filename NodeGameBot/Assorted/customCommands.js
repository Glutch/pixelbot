...
let areas = fs.readFileSync(
  path.resolve(__dirname, "../../../Database/lib/boxes.json")
);
...
//         console.log(
//           "Error: Gossip Window must be open to select a flight path."
//         );
//         return;
//       }
...
let createMacros = async () => {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < spellMacro.length; i++) {
      ...
      // Chooses Macro icon.
      leftClick(
        Math.floor(coords.x1) + (i % 5) * Math.floor(coords.x2),
        Math.floor(coords.y1) + Math.floor(i / 5) * Math.floor(coords.y2)
      );
...
let createImageOKAYButton = () => {
  return new Promise(async (res, rej) => {
    screenshot("../../StaticImages/casting.png", async (error, complete) => {
      ...
      image
        .crop(818, 914, 1101 - 818, 941 - 914)
        .write("../../StaticImages/casting.png");
      res();
    });
  });
};
...
Object.prototype.isDuplicate = function (object2) {
  ...
  for (propName in object2) {
    ...
    if (this[propName] instanceof Array && object2[propName] instanceof Array) {
      ...
      if (!this[propName].isDuplicate(object2[propName])) return false;
    } else if (
